...
import { formatDistance, formatDuration, maneuverArrow, maneuverLabel } from '@ijm/navigation';
    ...
    <div className="pointer-events-none absolute inset-x-0 top-0 z-20 flex justify-center px-3 safe-top">
      <div className="glass pointer-events-auto w-full max-w-[560px] rounded-[18px] px-4 py-3">
        <div className="flex items-center gap-4">
          <div
            className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl text-[30px] leading-none ${
              arrived
                ? 'bg-signal-500/20 text-signal-400'
                : 'bg-turn-500/20 text-turn-400'
            } ${camera.state === 'APPROACH_TURN' ? 'animate-pulse-soft' : ''}`}
          >
            {arrived ? '⚑' : maneuverArrow(next)}
          </div>
        ...
        {outlook.afterNext && !arrived && (
          <div className="mt-2 flex items-center gap-2 text-[12px] text-mist-500">
            ...
            <span>{maneuverArrow(outlook.afterNext)}</span>
