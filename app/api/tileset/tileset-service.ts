...
/**
 * 3D Tiles の tileset.json を「必要な範囲だけ」に絞って配信する処理の本体。
 *
 * 入口は 2 つある（どちらもここを呼ぶ）:
 *   app/api/tileset/[...spec]/route.ts  パスに条件を書く形（Cesium が読むのはこちら）
 *   app/api/tileset/route.ts            クエリに条件を書く形（旧来の入口）
 *
 *
...
/** 配信の条件。パスから読んでもクエリから読んでも、ここへまとめる */
export interface TilesetParams {
  city: string | null;
  /** near / far / detail / 追加レイヤの id */
  layer: string | null;
  /** 建物モデルの見え方。null なら都市の既定 */
  model: string | null;
  /** "minLng,minLat,maxLng,maxLat"。null なら都市の bbox */
  bbox: string | null;
}

export async function tilesetResponse(params: TilesetParams): Promise<Response> {
  const layer = params.layer ?? 'near';
   ...
   * テクスチャ無し版が配信されていない。
   */
  const model = resolveBuildingMode(city, params.model);
  /**
   * 試す順序。
   *
  ...
  const bbox = parseBBoxParam(params.bbox) ?? city.bbox;
