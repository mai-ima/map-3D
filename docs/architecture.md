...
`{area}-{feature}-{lod}` 形式のまとめ配信には、すべての市区町村が入っているわけではない。
浜松市は 22130 / 22131 のいずれでもまとめ配信が空になるが、
PLATEAU の GraphQL API を見ると**区ごとに個別の tileset.json が配信されている**。

このような都市は、`PlateauTilesetSpec.url` に配信 URL を直接書いて登録する。
URL 指定のデータセットは既に単一の区に絞られているため、
BFF での市区町村フィルタと LOD フォールバックは行わない
（children が地理的な四分木なので、絞り込むと穴が開く）。

なお、まとめ配信の子は絶対 URL だが、区ごとの配信は**相対 URI**（`data/data239.b3dm`）
で書かれている。tileset.json を自分のオリジンから配信すると相対 URI が
自分のオリジン基準で解決されてタイル本体が 404 になるため、
BFF は上流 URL を基準に絶対 URL へ書き換えてから返す。

### 4.3 整備状況の調べ方

都市を追加するときは、次のコマンドで実際の配信を確認してから
`packages/shared/src/cities.ts` に反映する。

```
npm run survey:lod           # LOD・地物タイプの整備状況（都道府県単位）
npm run survey:lod -- 13 14  # 都道府県コードを直接指定
npm run survey:city -- 22130 # GraphQL API で市区町村ごとの配信 URL を取得
```
