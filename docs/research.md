...
**訂正（2026-08-29 の再監査）: 毎フレームの `gl.flush()` は入れて、そのあと外した。**
WebKit の推奨は「大量の描画コマンドを**途中で**分割せよ」だが、
実装した場所（Cesium の `postRender`）はフレーム描画の**最後**で、
そこで flush してもコマンドの分割にならない。
ブラウザは元々フレーム末尾で暗黙に flush するため、
追加の flush はパイプラインをストールさせるだけだった。
**残っている対応は以下の 3 つで、flush は使っていない。**

1. **iOS 既定でポストプロセスを外す**。MSAA・HDR・環境光遮蔽・ブルームは
   1 パスごとに描画コマンドを積み増すため、この上限に直接効く。
   Retina では 1 画素が小さく寄与も見えにくいので、失うものが少ない。
   **影がとくに効く**（シャドウパスでジオメトリをもう一度描くので描画コマンドがほぼ倍になる）
2. **Retina 解像度（`resolutionScale` 2.0）は維持**。見た目の精細さはここが効くので、
   品質の要は落としていない。ポストプロセスは「表示設定 → 描画品質 → 高品質」で有効にできる
3. **コンテキスト喪失時は自動でセーフモードに開き直す**（`sessionStorage` で 1 回に制限）

そもそも「1 フレームの描画コマンドが多すぎる」ことが原因なので、
**根本の対処は描く量を減らすこと**だった。
起動直後に都道府県まるごとの tileset を展開していた件（`/api/tileset` で解消）が
最も大きく効いている。
